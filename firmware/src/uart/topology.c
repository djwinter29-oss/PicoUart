...
static bool uart_topology_pin_valid(uint32_t pin, uint32_t gpio_count)
{
    return (pin != UART_TOPOLOGY_PIN_UNASSIGNED) && (pin < gpio_count);
}
...
bool uart_topology_validate(const uart_topology_port_t *ports, size_t port_count)
{
    ...
    for (size_t index = 0u; index < port_count; ++index) {
        ...
        if ((port->id != (uart_port_id_t)index) ||
            !uart_topology_pin_valid(port->tx_pin, port->target_gpio_count) ||
            !uart_topology_pin_valid(port->rx_pin, port->target_gpio_count) ||
        ...
        if (port->rts_enabled) {
            if (!uart_topology_pin_valid(port->rts_pin, port->target_gpio_count)) {
                return false;
            }
            pins[pin_count++] = port->rts_pin;
        }
        if (port->cts_enabled) {
            if (!uart_topology_pin_valid(port->cts_pin, port->target_gpio_count)) {
                return false;
            }
