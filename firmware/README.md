...
- Hardware UART ports accept supported baud/data/parity/stop updates and leave
  ...
  strings `CDC2`–`CDC5 PIO 8N1`) with docs-reserved RTS/CTS pins (not GPIO-owned).
...
- After init, core 0 arms an 8 s watchdog (pause-on-debug) and pets it in the USB poll loop.
