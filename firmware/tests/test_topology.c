...
static void test_topology_make_valid(uart_board_topology_port_t ports[UART_PORT_COUNT])
{
    for (size_t index = 0u; index < UART_PORT_COUNT; ++index) {
        bool is_hardware = index < 2u;
        uint32_t state_machine = (uint32_t)((index % 2u) * 2u);

        ports[index] = (uart_board_topology_port_t){
            ...
            .rts_pin = UART_BOARD_TOPOLOGY_PIN_UNASSIGNED,
            .cts_pin = UART_BOARD_TOPOLOGY_PIN_UNASSIGNED,
        };
    }
}
...
{
    uart_board_topology_port_t ports[UART_PORT_COUNT];

    test_topology_make_valid(ports);
    TEST_ASSERT_TRUE(uart_board_topology_validate(ports, UART_PORT_COUNT));
}
...
void test_rejects_wrong_port_count_or_identity(void)
{
    ...
    test_topology_make_valid(ports);
    TEST_ASSERT_FALSE(uart_board_topology_validate(NULL, UART_PORT_COUNT));
    TEST_ASSERT_FALSE(uart_board_topology_validate(ports, UART_PORT_COUNT - 1u));
    ports[3].id = UART_PORT_2;
    TEST_ASSERT_FALSE(uart_board_topology_validate(ports, UART_PORT_COUNT));
}
...
void test_rejects_invalid_pin_assignments(void)
{
    ...
    test_topology_make_valid(ports);
    ports[0].tx_pin = UART_BOARD_TOPOLOGY_PIN_UNASSIGNED;
    TEST_ASSERT_FALSE(uart_board_topology_validate(ports, UART_PORT_COUNT));
