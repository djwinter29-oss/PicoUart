...
if [ "$PICO_SDK_VERSION_PINNED" -eq 0 ]; then
    ...
    PICO_SDK_VERSION=$(git ls-remote --tags --refs "$PICO_SDK_REPOSITORY" 'refs/tags/[0-9]*' |
        awk -F/ '{print $3}' |
        grep -E '^[0-9]+\.[0-9]+\.[0-9]+$' |
        sort -V |
        tail -n 1)
...
if [ ! -d "$PICO_SDK_PATH_VALUE" ]; then
    ...
    if ! git -C "$PICO_SDK_PATH_VALUE" fetch --depth 1 --force origin \
        "refs/tags/$PICO_SDK_VERSION:refs/tags/$PICO_SDK_VERSION"; then
        echo "Pico SDK update check failed." >&2
        return 1 2>/dev/null || exit 1
    fi

    CURRENT_SDK_REVISION=$(git -C "$PICO_SDK_PATH_VALUE" rev-parse HEAD 2>/dev/null)
    LATEST_SDK_REVISION=$(git -C "$PICO_SDK_PATH_VALUE" rev-parse "$PICO_SDK_VERSION^{commit}" 2>/dev/null)
    if [ "$CURRENT_SDK_REVISION" != "$LATEST_SDK_REVISION" ]; then
        echo "Updating Pico SDK checkout to $PICO_SDK_VERSION"
        if ! git -C "$PICO_SDK_PATH_VALUE" checkout --detach "$PICO_SDK_VERSION"; then
            echo "Pico SDK checkout update failed." >&2
            return 1 2>/dev/null || exit 1
        fi
    else
        echo "Pico SDK is already at $PICO_SDK_VERSION"
    fi
